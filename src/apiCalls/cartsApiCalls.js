   ...
   */
    if (itemToUpdate) {
      const response = await axios.patch(
        `${process.env.REACT_APP_SERVER_API}/carts/${cartId}`,
        {
    ...
    const response = await axios.post(`${process.env.REACT_APP_SERVER_API}/carts/${cartId}`, {
      record: record._id,
      quantity: 1,
    });
...
export const getCartData = async (dispatchCarts, cartId) => {
  try {
    const response = await axios.get(`${process.env.REACT_APP_SERVER_API}/carts/${cartId}`);
...
export const deleteCartItem = async (dispatchCarts, recordId, cartId) => {
  try {
    await axios.put(`${process.env.REACT_APP_SERVER_API}/carts/${cartId}`, {
      record: recordId,
    });
